...
type tApiReply<'a> = Belt.Result.t<'a, ApiError.t> 

let apiCreateUser = (payload: Js.Json.t): Js.Promise.t<tApiReply<unit>> => {
  let cFUN = "apiCreateUser()";

  let createNewUser = (user: LibWeb.Rest.User.t): Js.Promise.t<tApiReply<unit>>  => {
    ResPg.Pool.query("insert into users(id, user_name, user_email, passowrd) values($1, $2, $3, $4)", [ResPg.Pg.Query.string(ResUuid.Uuid.id()), ResPg.Pg.Query.string(user.userName), ResPg.Pg.Query.string(user.userEmail), ResPg.Pg.Query.string(ResCrypto.Crypto.sha256(user.password))])  
    -> Js.Promise.then_((_) => {
    ...
    -> Js.Promise.catch((e) => {
      ...
      Js.Promise.resolve(Belt.Result.Error({ApiError.err: "error", message: "error"}))
    },_)
  } 
  ...
  let verifyUserName = (user: LibWeb.Rest.User.t): Js.Promise.t<tApiReply<unit>> => {

    ResPg.Pool.query("select count(*) from users where user_name = $1", [ResPg.Pg.Query.string(user.userName)])  
    -> Js.Promise.then_((result: ResPg.Pg.Query.result<CountResult.t>) => {
      if result.rows[0].count > 0 {
          Js.Promise.resolve(Belt.Result.Error({ApiError.err: "user_already_exists", message: "user already exists"}))
      } else {
        Js.Promise.resolve(Belt.Result.Ok(()))
      }
    },_) 
  ...
  let verifyEmail = (user: LibWeb.Rest.User.t): Js.Promise.t<tApiReply<unit>> => {
    ResPg.Pool.query("select count(*) from users where email = $1", [ResPg.Pg.Query.string(user.userEmail)])  
    -> Js.Promise.then_((result: ResPg.Pg.Query.result<CountResult.t>) => {
        if (result.rows[0].count > 0) {
          Js.Promise.resolve(Belt.Result.Error({ApiError.err: "email_already_exists", message: "email already exists"}))
        } else {
          Js.Promise.resolve(Belt.Result.Ok(()))
        }
      },_) 
  ...
  | None =>
    Js.Promise.resolve(Belt.Result.Error({ApiError.err: "could_not_parse_user", message: "could not parse user"}))
  }
}
